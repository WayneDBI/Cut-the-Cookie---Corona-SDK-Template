...
function scene:createScene( event )
	...
	-- *********************************************************************************************************************************
    HeroTarget	= Hero.newHero(   {  positionX=160,	positionY=130  }   )	-- Hero: Set X, Y position on the screen.
	Cookie1 	= Cookie.newCookie( 160, 360, 240, 0) 						-- X, Y, [Initial VelocityX, VelocityY]
    Star1 		= Star.newStar( 160,420 )									-- Add Star 1 to the scene
    Star2 		= Star.newStar( 160,260 )									-- Add Star 2 to the scene
    Star3 		= Star.newStar( 160,200 )									-- Add Star 3 to the scene
	-- *********************************************************************************************************************************
	-- *******   ADD YOUR LEVEL DATA BELOW, ROPES, BUBBLES, SPIKES ETC..  **************************************************************
	-- *********************************************************************************************************************************
	-- Add ROPES, AUTO-ROPES, SPIKES and BUBBLES as required
	-- *********************************************************************************************************************************
	local Rope1 = Rope.newRope( {extraLength=5,	nubX=40,	nubY=200,	targetX=Cookie1.x,	targetY=Cookie1.y,	body=Cookie1,	density=120} )
	local Rope2 = Rope.newRope( {extraLength=5,	nubX=280,	nubY=200,	targetX=Cookie1.x,	targetY=Cookie1.y,	body=Cookie1,	density=120} )

    local Bubble1 = Bubble.newBubble(100,400)
    local Bubble2 = Bubble.newBubble(160,410)
    local Bubble3 = Bubble.newBubble(220,400)
   
   	local Spike1 		= Spike.newSpike( {spikeType=2,	x=60,	y=80,	getAngle=0,		doRotate=true,	rotateSpeed=2}	) -- spikeType (1 to 4), x, y, getAngle, doRotate, rotateSpeed
    local Spike2 		= Spike.newSpike( {spikeType=2,	x=280,	y=80,	getAngle=0,		doRotate=true,	rotateSpeed=-2}	) -- spikeType (1 to 4), x, y, getAngle, doRotate, rotateSpeed
